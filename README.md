...
## Ограничения SOCKS5-прокси

SOCKS5 проксирует **только TCP-трафик**. Всё что использует UDP — либо не работает, либо переключается на TCP с потерей качества.

### Не работает через SOCKS5

| Категория | Примеры | Причина |
|-----------|---------|---------|
| Голосовые звонки | Discord, Telegram, WhatsApp, Zoom, Google Meet | WebRTC / UDP |
| Онлайн-игры | Большинство мультиплеера | UDP для реалтайм-данных |
| Торренты (частично) | DHT, UDP-трекеры | UDP; TCP-трекеры и пиры работают |
| Ping | `ping google.com` | ICMP — не TCP |
| Нативные приложения | Некоторые десктопные программы | Игнорируют системный прокси |

### Работает через SOCKS5

| Категория | Примеры |
|-----------|---------|
| Веб-сайты | Всё что в браузере через HTTPS |
| Мессенджеры (текст) | Discord, Telegram, Twitter — текст и картинки |
| Стриминг | YouTube, Twitch, Spotify (TCP-фолбек) |
| QUIC / HTTP3 | YouTube, Google — браузер автоматически переключается на TCP |
| API, SSH, Git | Любой TCP-трафик |

> **Если нужен полный охват (включая UDP и голосовые звонки)** — это задача для VPN (WireGuard, OpenVPN), а не прокси-туннеля. proxyebator предназначен для маскировки веб-трафика.
