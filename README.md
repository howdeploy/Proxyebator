...
```bash
# Скачать скрипт
curl -fLO https://raw.githubusercontent.com/USER/REPO/main/proxyebator.sh
chmod +x proxyebator.sh

# Интерактивная установка — скрипт задаст вопросы
sudo ./proxyebator.sh server

# Или без вопросов (AI-friendly)
sudo ./proxyebator.sh server --domain example.com --tunnel chisel
```

После завершения установки скрипт выведет команду для клиента. Сохрани её — она понадобится на своём компьютере.
...
<details>
<summary><b>Настройка SOCKS5 в GUI-клиентах</b></summary>
...
> Скопируй весь блок ниже и отправь Claude, ChatGPT или другому AI-ассистенту — он развернёт proxyebator на твоём сервере без дополнительных вопросов.
...
7. Сохрани строку с командой для клиента из вывода скрипта

Требования к серверу:
- Debian 12+, Ubuntu 22.04+, CentOS 8+, AlmaLinux 8+, Rocky Linux 8+, Fedora 38+, или Arch Linux
- Открытые порты: 80/tcp (для certbot) и 443/tcp (для туннеля)
- Домен с A-записью, указывающей на IP этого сервера (серое облако Cloudflare, не оранжевое!)
- Root-доступ или sudo

Что НЕ делать:
- Не включать оранжевое облако Cloudflare — WebSocket не пройдёт через CDN, соединение будет рваться каждые ~100 секунд
- Не запускать без домена — TLS-сертификат нужен для маскировки трафика
- Не менять порт туннеля вручную — скрипт назначает 7777 (chisel) или 7778 (wstunnel) автоматически

После завершения установки запусти клиент на своём компьютере командой из вывода сервера,
затем проверь: curl --socks5-hostname localhost:1080 https://ifconfig.me
