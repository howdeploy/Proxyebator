...
Note: wstunnel client does NOT need --auth or user/pass. The secret path in the URL is the only auth. So client_collect_interactive and client_collect_params should make CLIENT_PASS optional when tunnel type is wstunnel. For simplicity, leave the existing param collection as-is but don't use CLIENT_PASS in client_run_wstunnel.
