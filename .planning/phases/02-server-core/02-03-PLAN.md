...
4. **Two-pass approach for TLS:**
   - If cert already exists (`[[ -f "/etc/letsencrypt/live/${DOMAIN}/fullchain.pem" ]]`): write the full SSL config directly (calls write_nginx_ssl_config defined in Task 2 of this plan), test with `nginx -t`, reload nginx. Done.
   - If cert does NOT exist: write a minimal HTTP-only config (listen 80, server_name, basic location /) so nginx can serve port 80 for certbot ACME challenge. Reload nginx. server_obtain_tls (Task 2) will run certbot, then call write_nginx_ssl_config() to overwrite with the full SSL + WebSocket + masquerade config and reload nginx again.
   - NOTE: write_nginx_ssl_config() is implemented in Task 2 of this plan. server_configure_nginx() calls it only when cert already exists; otherwise server_obtain_tls() calls it after certbot succeeds.
  </action>
  <verify>
