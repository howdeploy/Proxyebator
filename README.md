...
> Скопируй блок ниже и отправь Claude, ChatGPT или другому AI-ассистенту.
> AI-ассистент должен иметь **SSH-доступ к твоему VPS** — скрипт запускается **на сервере**, а не локально.

```
Задача: развернуть proxyebator — маскирующий WebSocket-прокси — на моём Linux VPS.

Где запускать: все команды ниже выполняются НА СЕРВЕРЕ через SSH.
AI-ассистент должен подключиться к серверу по SSH и выполнить команды там.
...
5. Сохрани строку с командой для клиента из вывода скрипта — она понадобится на ЛОКАЛЬНОМ компьютере
...
Что дальше (на ЛОКАЛЬНОЙ машине пользователя):
- Скачать тот же скрипт и запустить клиент командой из вывода сервера
- Проверить: curl --socks5-hostname localhost:1080 https://ifconfig.me
- Должен вернуть IP сервера, а не домашний IP
